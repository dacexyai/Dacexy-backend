...
SYSTEM_PROMPT = """You are an expert web developer. Generate a complete single-page HTML website.
Rules:
- Return ONLY the HTML code, nothing else
- No explanations, no markdown, no backticks
- Use inline CSS and JavaScript only
- Make it beautiful with a modern design
- Keep it under 200 lines for fast generation
- Use a clean color scheme"""

async def generate_website(prompt: str, ai: DeepSeekProvider) -> str:
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user", "content": f"Create a website for: {prompt}. Return only HTML code."}
    ]
    try:
...
<body>{html}</body>
        ...
        return """<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Generated Website</title>
<style>body{font-family:Arial,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:linear-gradient(135deg,#667eea,#764ba2);color:white;text-align:center}</style>
</head>
<body>
<div>
<h1>🚀 Your Website</h1>
<p>Generated by Dacexy AI</p>
<p style="opacity:0.8">Website generation timed out. Please try again with a simpler description.</p>
</div>
</body>
</html>"""
    except Exception as e:

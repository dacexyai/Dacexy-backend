...
SYSTEM_PROMPT = "You are an expert web developer. Generate a complete single-page HTML website. Return ONLY the HTML code, nothing else. No explanations, no markdown, no backticks. Use inline CSS and JavaScript only. Make it beautiful with a modern design. Keep it under 200 lines."

FALLBACK_HTML = "<html><head><title>Website</title></head><body style='font-family:Arial;text-align:center;padding:50px'><h1>Your Website</h1><p>Generated by Dacexy AI</p><p>Generation timed out. Please try again.</p></body></html>"

async def generate_website(prompt: str, ai: DeepSeekProvider) -> str:
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user", "content": "Create a website for: " + prompt + ". Return only HTML code, no explanation."}
    ]
    try:
        ...
        if not html.strip().startswith("<!") and not html.strip().startswith("<html"):
            html = "<html><head><meta charset='UTF-8'><title>Generated Site</title></head><body>" + html + "</body></html>"
        return html

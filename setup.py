...
SYSTEM_PROMPT = "You are an expert web developer. Generate a complete single-page HTML website. Rules: Return ONLY the HTML code, nothing else. No explanations, no markdown, no backticks. Use inline CSS and JavaScript only. Make it beautiful with a modern design. Keep it under 200 lines for fast generation."
...
<style>
body { font-family: Arial, sans-serif; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; background: linear-gradient(135deg, #667eea, #764ba2); color: white; text-align: center; }
...
<div class="card">
<h1>Your Website</h1>
<p>Generated by Dacexy AI</p>
<p>Website generation timed out. Please try again with a simpler description.</p>
</div>
</body>
</html>"""
...
async def generate_website(prompt: str, ai: DeepSeekProvider) -> str:
    messages = [
        ...
        {"role": "user", "content": "Create a website for: " + prompt + ". Return only HTML code."}
    ]
    try:
        ...
        if not html.strip().startswith("<!DOCTYPE") and not html.strip().startswith("<html"):
            html = "<!DOCTYPE html><html lang=\\"en\\"><head><meta charset=\\"UTF-8\\"><title>Generated Website</title></head><body>" + html + "</body></html>"
        return html

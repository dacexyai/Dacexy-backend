...
def build_template(prompt: str, name: str, ud: dict) -> str:
    ...
    hov = "0 20px 60px rgba(0,0,0,0.35)" if is_dark else "0 20px 60px rgba(0,0,0,0.1)"
    ...
    svcs = "".join([f\'<div class="sc" onmouseover="this.style.transform=\'translateY(-8px)\';this.style.borderColor=\\\'{ds["pr"]}\\\'" onmouseout="this.style.transform=\'\';this.style.borderColor=\\\'{ds["br"]}\'"><span class="si">{ic}</span><h3>{t}</h3><p>{d}</p></div>\' for ic,t,d in con["sv"]])
    stats = "".join([f\'<div class="sti"><div class="sn">{n}</div><div class="sl">{l}</div></div>\' for n,l in con["stats"]])
    testis = "".join([f\'<div class="tc" onmouseover="this.style.transform=\'translateY(-6px)\';this.style.borderColor=\\\'{ds["pr"]}\\\'" onmouseout="this.style.transform=\'\';this.style.borderColor=\\\'{ds["br"]}\'"><div class="ts">★★★★★</div><p class="tt">"{t}"</p><div class="ta"><div class="av">{a[0]}</div><div><div class="an">{a}</div><div class="ar">{r}</div></div></div></div>\' for a,r,t in con["testi"]])
    gals = "".join([f\'<div class="gi" onmouseover="this.querySelector(\'img\').style.transform=\'scale(1.1)\'" onmouseout="this.querySelector(\'img\').style.transform=\'\'"><img src="{imgs[k]}" loading="lazy" alt="Gallery"/><div class="go"><span>View →</span></div></div>\' for k in ["g1","g2","g3","g4"]])
    afs = "".join([f\'<div class="af" onmouseover="this.style.borderColor=\\\'{ds["pr"]}\\\';this.style.transform=\'translateX(5px)\'" onmouseout="this.style.borderColor=\\\'{ds["br"]}\\\';this.style.transform=\'\'"><div class="afi">{ic}</div><div class="aft"><h4>{t}</h4><p>{d}</p></div></div>\' for ic,t,d in con["af"]])

    social_links = ""
    if ig: social_links += f\'<a href="https://instagram.com/{ig}" target="_blank" title="Instagram">📸</a>\'
    else: social_links += \'<a href="#" title="Instagram">📸</a>\'
    if fb: social_links += f\'<a href="https://facebook.com/{fb}" target="_blank" title="Facebook">👍</a>\'
    else: social_links += \'<a href="#" title="Facebook">👍</a>\'
    if tw: social_links += f\'<a href="https://twitter.com/{tw}" target="_blank" title="Twitter">🐦</a>\'
    else: social_links += \'<a href="#" title="Twitter">🐦</a>\'
    if li: social_links += f\'<a href="https://linkedin.com/in/{li}" target="_blank" title="LinkedIn">💼</a>\'
    else: social_links += \'<a href="#" title="LinkedIn">💼</a>\'
...
.mob{{display:none;position:fixed;inset:0;z-index:999;background:{"rgba(2,0,8,0.98)" if is_dark else "rgba(255,255,255,0.99)"};backdrop-filter:blur(30px);flex-direction:column;align-items:center;justify-content:center;gap:28px}}
.mob.o{{display:flex}}
...
.fg{{display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:44px;max-width:1280px;margin:0 auto;margin-bottom:36px}}
...
.ckb{{position:fixed;bottom:0;left:0;right:0;z-index:9970;background:{"rgba(2,0,8,0.97)" if is_dark else "rgba(15,15,15,0.97)"};color:#fff;padding:14px 5%;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;transition:transform 0.4s}}
...
@media(max-width:900px){{
  ...
  .fg2g{{grid-template-columns:1fr}}
}}
@media(max-width:540px){{
  .stin,.fg{{grid-template-columns:1fr}}
  ...
  <div class="hin">
    ...
    <div class="hiw">
      <div class="hic">
        ...
        <div class="hib"><div class="ldot"></div><span class="ltext">Live · {hours}</span></div>
      </div>
    </div>
  </div>
</section>
...
<section id="contact" style="background:{svc_bg}">
  <div class="sec">
    ...
    <div class="ccg rev">
      ...
      <div class="cc"><div class="cci">✉️</div><h4>Email Us</h4><a href="mailto:{email_addr}" style="word-break:break-all;font-size:0.76rem">{email_addr}</a></div>
...
<footer style="padding:60px 5% 90px;border-top:1px solid var(--br);background:{svc_bg}">
  <div class="fg" style="max-width:1280px;margin:0 auto;margin-bottom:36px;display:grid;grid-template-columns:2fr 1fr 1fr 1fr;gap:44px">
    <div class="fbr">
    ...
    <div class="fc">
      <h4>Services</h4>
      {"".join([f\'<a href="#services">{s[1]}</a>\' for s in con["sv"]])}
    </div>
    <div class="fc">
      <h4>Contact</h4>
...
async def generate_website(prompt: str, ai=None) -> str:
    ...
    # Use AI only for genuinely custom/complex requests — saves API credits

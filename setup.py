...
SYSTEM_PROMPT = "You are an expert web developer. Generate complete, beautiful, production-ready single-page HTML websites. Always return ONLY valid HTML with embedded CSS and JavaScript. Do not include any explanation - only the HTML code."

async def generate_website(prompt, ai):
    messages = [
...
async def run_agent(body: AgentRunRequest, user: User = Depends(_get_current_user), db: AsyncSession = Depends(get_db), ai: DeepSeekProvider = Depends(get_deepseek)):
    messages = [{"role": "system", "content": "You are an autonomous AI agent. Break down tasks and execute them step by step."}, {"role": "user", "content": "Task: " + body.task + ("\nContext: " + body.context if body.context else "")}]
...
subprocess.run([sys.executable, "-m", "uvicorn", "src.main:app", "--host", "0.0.0.0", "--port", port], check=True)

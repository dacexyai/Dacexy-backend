"""
╔══════════════════════════════════════════════════════════════════════════════╗
║         DACEXY DESKTOP AGENT v14.0 ULTIMATE — STARTUP HARDENED             ║
║  Fix: CMD flash-close, startup errors captured, WebSocket resilient        ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# ════════════════════════════════════════════════════════════════════════════
# BLOCK 2 — AUTO-INSTALL DEPENDENCIES
# ════════════════════════════════════════════════════════════════════════════
import subprocess
        ...
        try:
            ...
            if "pipwin" in _cmd:
                subprocess.check_call(
                    ...
                    timeout=60)
            import pyaudio; PYAUDIO_OK = True
            log.info("PyAudio installed OK")
            break
        except Exception:
            continue

# ════════════════════════════════════════════════════════════════════════════
# BLOCK 3 — ALL IMPORTS (each wrapped so one failure doesn't kill agent)
# ════════════════════════════════════════════════════════════════════════════
import asyncio, base64, io, json, threading, time
import webbrowser, re, ctypes, queue, smtplib
import socket, hashlib, traceback, random, zipfile, csv
...
try:
    ...
    # Create stub so agent doesn't crash
...
class AgentStatus(Enum):
    IDLE=auto(); PLANNING=auto(); EXECUTING=auto(); VERIFYING=auto()
    RETRYING=auto(); PAUSED=auto(); ERROR=auto(); STOPPED=auto()
...
def check_token_valid(token: str) -> bool:
    ...
    try:
        ...
        log.warning("Token check failed (no internet?): %s", e)
        return False
...
def login() -> Optional[str]:
    ...
    if not password or len(password) < 4:
        _err("Password too short (min 4 chars)")
        return None

    if not req_lib:
        _err("requests library not available")
        return None
...
class MemorySystem:
    def __init__(self):
        ...
        try:
            ...
            log.warning("Memory initial load failed (starting fresh): %s", e)
    ...
    def remember_success(self, task: str, method: str, dur: float = 0):
        self.store(f"SUCCESS: {task} via {method}", "success",
    ...
    def load(self):
        try:
            ...
            log.warning("Memory load error (non-fatal): %s", e)
...
def remember(fact: str, category: str = "fact"):
    try: get_mem().store(fact, category)
    except Exception: pass
...
def get_memory_context(query: str = "") -> str:
    try: return get_mem().get_context(query)
...
class SuperVisionEngine:
    ...
    def capture(self, region=None, quality: int = 75) -> Optional[str]:
        try:
            ...
            log.debug("Vision capture: %s", e); return None

    def capture_pil(self, region=None) -> Optional[Any]:
        try:
            if ImageGrab is None: return None
            img = ImageGrab.grab(bbox=region) if region else ImageGrab.grab()
            with self._lock:
                self._last_screen = img
            return img
        except Exception as e:
            log.debug("capture_pil: %s", e); return None
    ...
    def get_monitors(self) -> List[Dict]:
        ...
        if not monitors:
            ...
            monitors.append({"left":0,"top":0,"right":w,
                             "bottom":h,"width":w,"height":h})
        return monitors
    ...
    def ocr_image(self, path: str) -> str:
        if not TESSERACT_OK or Image is None: return ""
        try:
            img = Image.open(path)
            return pytesseract.image_to_string(img).strip()
        except Exception as e:
    ...
    def ocr_from_pdf(self, path: str) -> str:
        try:
            import PyPDF2
        ...
        try:
            ...
            for i, word in enumerate(data["text"]):
                if search.lower() in str(word).lower() \
                    ...
                    if region: x+=region[0]; y+=region[1]
                    return (x, y)
        except Exception as e:
        ...
        try:
            ...
            for cnt in contours:
                ...
                if 300 < area < 150000 and 0.05 < ratio < 25:
                    et = "button" if 18<h<55 and 30<w<400 else \
                         "input" if h<35 and w>80 else "unknown"
                    cx = x+w//2+(region[0] if region else 0)
                    cy = y+h//2+(region[1] if region else 0)
                    elements.append(UIElement(
                        label=et,x=cx,y=cy,width=w,height=h,
                        elem_type=et,confidence=min(1.0,area/25000)))
            with self._lock:
    ...
    def detect_forms(self) -> List[Dict]:
        return [asdict(e) for e in self.detect_ui_elements()
                if e.elem_type in ("input","button")]

    def detect_popups(self) -> bool:
        text = self.ocr_fast().lower()
        sigs = ["ok","cancel","yes","no","close","error",
                "warning","alert","confirm"]
        return sum(1 for s in sigs if s in text) >= 2

    def detect_error_dialogs(self) -> Optional[str]:
        text = self.ocr_fast().lower()
        errs = ["error","failed","cannot","invalid","not found",
    ...
    def find_button(self, label: str) -> Optional[UIElement]:
        for e in self.detect_ui_elements():
            if label.lower() in e.text.lower(): return e
    ...
    def get_ai_description(self, token: str) -> str:
        ...
        if not ss: return self.ocr_fast()[:300] or "Screen captured"
        try:
            if not req_lib: return self.ocr_fast()[:300]
            r = req_lib.post(
    ...
    def find_image_on_screen(self, template_path: str,
        ...
        try:
            ...
            sg = cv2.cvtColor(np.array(screen.convert("RGB")),
                              cv2.COLOR_RGB2GRAY)
...
def ocr_find_text(text:str, region=None):
...
def detect_buttons_on_screen() -> List[Dict]:
    return [asdict(e) for e in get_vision().detect_ui_elements()
            if e.elem_type=="button"]
def get_screen_text_via_ai(token:str) -> str:
    return get_vision().get_ai_description(token)

# ════════════════════════════════════════════════════════════════════════════
# BLOCK 12 — MOUSE & KEYBOARD
# ════════════════════════════════════════════════════════════════════════════
...
def press_key(key:str):
    ...
    except Exception as e: log.debug("press_key: %s", e)

def hotkey(*keys):
    try: pyautogui.hotkey(*keys)
    except Exception as e: log.debug("hotkey: %s", e)

def hold_key(key:str, duration:float=1.0):
    try:
        pyautogui.keyDown(key); time.sleep(duration)
        pyautogui.keyUp(key)
    except Exception as e: log.debug("hold_key: %s", e)

def get_clipboard() -> str:
    try: return pyperclip.paste() if pyperclip else ""
    except Exception: return ""

def set_clipboard(text: str):
    try:
        if pyperclip: pyperclip.copy(str(text))
    except Exception: pass
...
def open_app(app_name:str) -> bool:
    try:
        ...
        try:
            ...
            log.warning("open_app: %s", e); return False

# ════════════════════════════════════════════════════════════════════════════
# BLOCK 14 — FILE ENGINE
# ════════════════════════════════════════════════════════════════════════════
class FileEngine:
    SUPPORTED = {
        "text":  [".txt",".md",".py",".js",".html",".css",".xml",
        ...
        try:
            ...
            if ext==".docx" and DOCX_OK:
                ...
                with open(str(p),mode,encoding="utf-8") as f:
                    f.write(content)
            audit("WRITE_FILE",path,"OK"); return True
        except Exception as e:
            log.warning("write_file: %s",e); return False
    ...
    def delete(self, path: str, safe: bool=True) -> bool:
        ...
        try:
            if safe:
                ...
                p.rename(dest); audit("DELETE_SAFE",path,str(dest))
            else:
    ...
    def organize_folder(self,folder:str)->Dict[str,int]:
        ...
        base  = Path(folder)
        tmap  = {ext:cat for cat,exts in self.SUPPORTED.items()
                 for ext in exts}
        try:
...
def list_files(folder=None,pattern="*"):
    return get_file_engine().list_files(folder,pattern)
...
def search_files(kw,folder=None,ext=None):
    return get_file_engine().search(kw,folder,ext)
...
class EmailCampaignManager:
    ...
    def _get_smtp(self, idx: int = 0):
        pool = self.smtp_pool if self.smtp_pool else (
            [self.smtp_config] if self.smtp_config else [])
        if not pool:
            raise ValueError(
                "SMTP not configured. "
    ...
    def create_campaign(self, name:str, subject:str, body_template:str,
        ...
        camp = EmailCampaignData(
            ...
            html=html,delay_sec=delay_sec,
            scheduled_at=scheduled_at,tags=tags or [])
        with _campaign_lock:
    ...
    def send_campaign(self, campaign_id: str,
        ...
        speak(f"Starting campaign '{camp.name}' "
    ...
    def send_single(self,to:str,subject:str,body:str,
        ...
        try:
            msg = MIMEMultipart("alternative")
            msg["Subject"]=subject; msg["From"]=self.smtp_config.get("email","")
            msg["To"]=to
            ...
            s.sendmail(cfg["email"],to,msg.as_string()); s.quit()
            audit("SEND_EMAIL",mask_pii(to),"OK"); return True
        except Exception as e:
    ...
    def get_analytics(self, campaign_id: str) -> Dict:
        ...
        return {
            "name":camp.name,"total":total,
            "sent":camp.sent,"failed":camp.failed,
            "opened":camp.opened,"status":camp.status,
...
class EnterpriseBrowserAgent:
    ...
    def _by(self, by: str):
        return {
            ...
            "name":By.NAME,"text":By.LINK_TEXT,
    ...
    def find_and_click(self, selector: str, by: str = "css",
        ...
        sels = [(selector,by)] + \
               [(s,"css") for s in (fallback_selectors or [])]
        for sel,b in sels:
            try:
    ...
    def find_and_type(self, selector: str, text: str,
        ...
        try:
            ...
            if clear: el.clear(); time.sleep(0.1)
    ...
    def login(self, url:str, email_sel:str, pass_sel:str,
        ...
        try:
            ...
            self.handle_captcha()
            self.find_and_type(email_sel, email)
            time.sleep(0.6+random.uniform(0,0.3))
            self.find_and_type(pass_sel, password)
            ...
            self._save_cookies()
            audit("BROWSER_LOGIN",mask_pii(email),"OK" if ok else "FAIL")
            return ok
        except Exception as e:
    ...
    def screenshot_b64(self) -> Optional[str]:
        try:
            return base64.b64encode(
                self.driver.get_screenshot_as_png()).decode()
        except Exception: return None
    ...
    def research_topic(self, topic: str, max_pages: int = 3) -> Dict:
        ...
        try:
            ...
            urls  = [e.get_attribute("href") for e in links
                     ...
                     "google.com" not in (e.get_attribute("href") or "")
                     ][:max_pages]
            for url in urls:
                try:
                    self.new_tab(url); time.sleep(3)
            ...
            try:
                if re.sub(r'\D','',contact):
                    ...
                    self.go_to(
                        ...
                        f"?phone={phone}&text={quote(message)}")
                    time.sleep(5)
                    ...
                    search = WebDriverWait(self.driver,10).until(
                        EC.presence_of_element_located((
                            By.XPATH,'//div[@data-testid="search"]')))
    ...
    def twitter_post(self, username:str, password:str,
                      text:str, media_path:str=None) -> bool:
        try:
            ...
            time.sleep(3)
            self._save_cookies()
            _ok("Tweet posted"); return True
        except Exception as e:
            log.error("Twitter: %s",e); return False

    def linkedin_post(self, username:str, password:str,
                       text:str) -> bool:
        try:
            ...
            if tb: tb.click(); time.sleep(0.3); tb.send_keys(text)
            self.find_and_click('//button[contains(text(),"Post")]',
                                  by="xpath")
            time.sleep(3); self._save_cookies()
            _ok("LinkedIn posted"); return True
        except Exception as e:
            log.error("LinkedIn: %s",e); return False

    def facebook_post(self, username:str, password:str,
                       text:str, page_id:str=None) -> bool:
        try:
            ...
            self.find_and_click(
                ...
                fallback_selectors=['[placeholder*="What"]',
                                    '[role="textbox"]'])
            time.sleep(2.5)
            self.find_and_type(
            ...
            time.sleep(3); self._save_cookies()
            _ok("Facebook posted"); return True
        except Exception as e:
    ...
    def instagram_post(self, username:str, password:str,
        ...
        try:
            ...
            for _ in range(2):
                ...
                time.sleep(2)
            ce = self.find(
                '//div[@aria-label="Write a caption..."]',by="xpath")
    ...
    def youtube_upload(self, video_path:str, title:str,
        ...
        try:
            ...
            self.handle_captcha()
            self.find_and_click('//ytcp-button[@id="create-icon"]',
                                  by="xpath")
            time.sleep(1.5)
            self.find_and_click(
                '//tp-yt-paper-item[@id="text-item-0"]',by="xpath")
            time.sleep(2.5)
...
BLOCKED_COMMANDS = [
    ...
    "mkfs","dd if=/dev/zero","rd /s /q c:\\","bcdedit",
    "reg delete hklm","net user administrator /active:yes",
]
...
class BaseAgent(ABC):
    ...
    @abstractmethod
    def run(self, task: Dict[str,Any]) -> Any: pass
...
class PlannerAgent(BaseAgent):
    ...
    def run(self, task: Dict[str,Any]) -> List[Dict]:
        ...
        try:
            if not req_lib: raise ValueError("no requests")
            ...
            prompt = (
                ...
                f'"type":"click|type|open|wait|browser|email|'
                f'whatsapp|social|screenshot|speak|system|file",'
                f'"params":{{}}}}]'
            )
            r = req_lib.post(
                f"{BACKEND_HTTP}/ai/chat",
...
class AgentSwarm:
    ...
    def dispatch(self, handler: Callable, task: Dict,
...
class OperatorEngine:
    ...
    def verify(self, expected: str = None,
        ...
        if expected:  return self.vision.wait_for_text(expected,timeout=5)
        return True
    ...
    def execute_with_verify(self, command: Dict,
        ...
        for attempt in range(max_retries):
            ...
            if self.verify(expected):
                ...
                get_mem().remember_success(
                    command.get("action",""),f"attempt {attempt+1}")
        ...
        get_mem().remember_failure(
            command.get("action",""),"verify failed")
        return last_result

# ════════════════════════════════════════════════════════════════════════════
# BLOCK 20 — HIBERNATION
# ════════════════════════════════════════════════════════════════════════════
...
class AutonomousScheduler:
    ...
    def start(self):
        def _loop():
            while _agent_running:
                try: self._tick()
                except Exception as e: log.debug("Scheduler tick: %s",e)
                time.sleep(30)
...
class SelfHealingEngine:
    ...
    def get_health(self) -> Dict[str,Any]:
        if not psutil: return {"healthy":True,"cpu":0,"ram":0,"disk":0}
        try:
    ...
    def _check(self):
        ...
        if len(self._metrics)>50: self._metrics = self._metrics[-30:]
    ...
    def start(self):
        def _loop():
            while _agent_running:
                try: self._check()
                except Exception as e: log.debug("SelfHealing: %s",e)
                time.sleep(60)
        threading.Thread(target=_loop,daemon=True,
...
class VoiceAssistant3:
    ...
    def __init__(self, token: str, command_callback: Callable):
        ...
        if VOICE_AVAILABLE and self.rec:
            ...
            log.warning("VoiceAssistant3: voice not available "
                        "(PYAUDIO_OK=%s, sr=%s)", PYAUDIO_OK, sr)
    ...
    def route(self, text: str) -> Dict[str,Any]:
        ...
        if t.startswith("click "):
            return {"action":"click_text",
                    "text":re.sub(r'^click (on )?','',t).strip()}
    ...
    def _voice_loop(self):
        ...
        while self.running and _agent_running:
            try:
                ...
                is_stop  = any(p in text for p in
                               ["stop dacexy","emergency stop","halt"])
                if is_stop:
                    emergency_stop()
                elif is_wake:
                    ...
                    if cmd_text:
                        ...
                        try: self.callback(self.route(cmd_text))
                        except Exception as e:
                            log.warning("Voice callback: %s",e)
                    else:
                        speak("Didn't catch that. Try again.")
            except Exception as e:
                log.debug("Voice loop: %s",e); time.sleep(1)
...
def run_macro(name:str, executor:Callable) -> bool:
    ...
    for step in steps:
        try: executor(step)
        except Exception as e: log.warning("Macro step: %s",e)
    return True
...
def get_system_info() -> Dict[str,Any]:
    if not psutil: return {"error":"psutil not available"}
    try:
        cpu  = psutil.cpu_percent(interval=0.3)
        ram  = psutil.virtual_memory()
...
def execute_command(cmd:dict, token:str=None,
    ...
    try:
        ...
        # ── VISION ────────────────────────────────────────────────────
            ...
            found = vi.wait_for_text(cmd.get("text",""),
            ...
            return {"status":"ok","elements":
                    [asdict(e) for e in vi.detect_ui_elements()]}
            ...
            vi.start_monitoring(float(cmd.get("interval",2.0)))
            return {"status":"ok"}
        ...
        # ── WINDOWS ───────────────────────────────────────────────────
            ...
            return {"status":"ok" if focus_window(
                cmd.get("title","")) else "not_found"}
        ...
        # ── APPS ──────────────────────────────────────────────────────
            ...
            return {"status":"ok","apps":
                    [a["name"] for a in list_running_apps()[:30]]}
        elif action=="open_browser":
            webbrowser.open(cmd.get("url","https://google.com"))
            return {"status":"ok"}
        ...
        # ── FILES ─────────────────────────────────────────────────────
            ...
            return {"status":"ok","files":fe.list_files(
                cmd.get("folder"),cmd.get("pattern","*"))}
        elif action=="read_file":
            return {"status":"ok","content":fe.read(cmd.get("path",""))}
        elif action=="write_file":
            return {"status":"ok" if fe.write(
                cmd.get("path",""),cmd.get("content","")) else "error"}
        elif action=="delete_file":
            return {"status":"ok" if fe.delete(
                cmd.get("path",""),cmd.get("safe",True)) else "error"}
        elif action=="copy_file":
            return {"status":"ok" if fe.copy(
                cmd.get("src",""),cmd.get("dst","")) else "error"}
        elif action=="move_file":
            return {"status":"ok" if fe.move(
                cmd.get("src",""),cmd.get("dst","")) else "error"}
        elif action=="create_folder":
            return {"status":"ok" if create_folder(
                cmd.get("path","")) else "error"}
            ...
            return {"status":"ok","result":fe.organize_folder(
                cmd.get("folder",""))}
        ...
        # ── EMAIL ─────────────────────────────────────────────────────
            ...
            if email_mgr: email_mgr.setup_gmail(
                cmd.get("email",""),cmd.get("app_password",""))
            return {"status":"ok"}
        elif action=="setup_outlook":
            if email_mgr: email_mgr.setup_outlook(
                cmd.get("email",""),cmd.get("password",""))
            return {"status":"ok"}
        elif action=="send_email":
            if email_mgr:
                ok = email_mgr.send_single(
                    cmd.get("to",""),cmd.get("subject",""),
        ...
        # ── BROWSER ───────────────────────────────────────────────────
            ...
            return {"status":"ok","results":browser.google_search(
                cmd.get("query",""))}
        ...
        # ── WHATSAPP ──────────────────────────────────────────────────
            ...
            return {"status":"ok","result":browser.whatsapp_bulk(
                contacts,cmd.get("message","Hello!"),
        ...
        # ── SOCIAL MEDIA ──────────────────────────────────────────────
            ...
            for p,cred in creds.items():
                if p=="twitter": results["twitter"]=browser.twitter_post(
                    cred["username"],cred["password"],text)
                if p=="linkedin":results["linkedin"]=browser.linkedin_post(
                    cred["username"],cred["password"],text)
                if p=="facebook":results["facebook"]=browser.facebook_post(
                    cred["username"],cred["password"],text)
        ...
        # ── SWARM ─────────────────────────────────────────────────────
            ...
            if swarm:
                def _e(c): return execute_command(
                    c,token,browser,email_mgr,swarm,operator,scheduler,fe)
                return {"status":"ok","result":swarm.plan_and_execute(
                    cmd.get("task",""),_e)}
        ...
        # ── HIBERNATION ───────────────────────────────────────────────
            ...
            hs  = HibernationSystem()
            ctx = hs.load()
            if ctx: return {"status":"ok","task":ctx.task_name,
                             "step":ctx.done_steps}
        ...
        # ── MEMORY ────────────────────────────────────────────────────
            ...
            results = get_mem().search(
                cmd.get("query",""),int(cmd.get("top_k",5)),
                cmd.get("category"))
            ...
            if steps:
                for s in steps:
                    execute_command(s,token,browser,email_mgr,
                                   swarm,operator,scheduler,fe)
                return {"status":"ok"}
            return {"status":"error","message":"Workflow not found"}
        elif action=="remember_contact":
            remember_contact(cmd.get("name",""),
                             cmd.get("email",""),cmd.get("phone",""))
            return {"status":"ok"}
        ...
        # ── MACROS ────────────────────────────────────────────────────
            ...
            def _e(c): return execute_command(
                c,token,browser,email_mgr,swarm,operator,scheduler,fe)
            return {"status":"ok" if run_macro(cmd.get("name",""),_e)
                    else "error"}
        ...
        try: get_mem().remember_failure(action,str(e)[:100])
        except Exception: pass
        ...
        try:
            ...
            if mtype=="ping":
                ...
                result = await loop.run_in_executor(
                    ...
                    lambda: execute_command(
                        cmd_data,token,browser,email_mgr,
                        swarm,operator,scheduler))
                try:
                ...
                if swarm and task_desc:
                    def _e(c): return execute_command(
                        c,token,browser,email_mgr,swarm,operator,scheduler)
                    loop   = asyncio.get_event_loop()
                    result = await loop.run_in_executor(
                        _executor,
            ...
            try:
                ...
                await ws.send(json.dumps({
                    ...
                    "features":["voice3","vision_super",
                                "browser_enterprise","email_enterprise",
                                "whatsapp","marketing","memory_vector",
                                "swarm10","operator","hibernation",
...
def register_hotkeys(voice: VoiceAssistant3 = None):
    ...
    try:
        ...
        _ok("Hotkeys: Ctrl+Shift+D/S/E/M/V/H")
    except Exception as e:
    ...
    while _agent_running:
        ...
        try:
            if il in ("stop","exit","quit","bye"):
                ...
                if email_mgr:
                    emails_raw = input("  Recipients (comma): ").strip()
                    subject    = input("  Subject: ").strip()
                    body       = input("  Body: ").strip()
                    delay      = float(
                        input("  Delay (sec) [1]: ").strip() or "1")
                    recips = [e.strip() for e in emails_raw.split(",")
                               if "@" in e]
                    cid = email_mgr.create_campaign(
                        "cli",subject,body,recips,delay_sec=delay)
                    email_mgr.send_campaign(cid)
                else:
                    _err("Email not configured. Run: setup gmail <email> <app_password>")
            elif il.startswith("whatsapp bulk"):
                cr = input("  Contacts (+phone or name, comma): ").strip()
                ...
                for pn in ["twitter","linkedin","facebook"]:
                    if input(f"  Include {pn}? (y/n): ").lower()=="y":
                        creds[pn]={
                            "username":input(f"  {pn} user: ").strip(),
                            "password":input(f"  {pn} pass: ").strip()}
                if not browser.driver: browser.start("chrome")
                for pn,cred in creds.items():
                    if pn=="twitter": browser.twitter_post(
                        cred["username"],cred["password"],text)
                    if pn=="linkedin":browser.linkedin_post(
                        cred["username"],cred["password"],text)
                    if pn=="facebook":browser.facebook_post(
                        cred["username"],cred["password"],text)
...
# ════════════════════════════════════════════════════════════════════════════
# BLOCK 30 — MAIN (fully hardened — never closes on error)
# ════════════════════════════════════════════════════════════════════════════
async def main_async(token: str):
    ...
    try:
        get_mem()
        log.info("Memory system OK")
    except Exception as e:
        log.warning("Memory init failed (non-fatal): %s",e)
    ...
    try:
        ...
        log.info("Operator/Scheduler/Healer created OK")
    except Exception as e:
        log.error("Support engines failed: %s",e)
        operator  = OperatorEngine(token,_exec)
        scheduler = AutonomousScheduler(_exec)
        healer    = SelfHealingEngine(_exec)
    ...
    try:
        ...
        if hib.has_saved():
            speak("Found a saved task from last session. "
    ...
    # ── INTERACTIVE SHELL (daemon thread) ─────────────────────────────
    try:
    ...
    try:
        await ws_connect_loop(token,browser,email_mgr,
                              swarm,operator,scheduler)
    except Exception as e:
...
def main():
    ...
    try:
        ...
        try: token = login_loop()
            ...
            log.error("Cannot authenticate"); return

...
if errorlevel 1 (
    echo.
    echo  Python not found. Downloading and installing Python 3.11 automatically...
    echo  Please wait, this may take 2-3 minutes...
    echo.

    :: Download Python installer silently using PowerShell
    ...
    if errorlevel 1 (
        echo.
        echo  Python was installed but PATH needs a terminal restart.
...
python -m pip install pyautogui pillow websockets requests speechrecognition pyttsx3 numpy psutil pyperclip plyer pygetwindow --quiet
if errorlevel 1 (
    echo.
    echo  WARNING: Some packages failed quietly. Retrying...
    python -m pip install pyautogui pillow websockets requests speechrecognition pyttsx3 numpy psutil pyperclip plyer pygetwindow
    if errorlevel 1 (
        echo.
